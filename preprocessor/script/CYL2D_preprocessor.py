...
def writegrid(direc,lev,naxes,Raxis,Paxis,Zaxis,nc):
    ...
    if (writevtk):
        ...
        print('DIMENSIONS %(0)d %(1)d %(2)d'%{'0':nr+1,'1':np+1,'2':nz+1}, file=fvtk1)
        print('POINTS %(0)d float'%{'0':(nr+1)*(nz+1)*(np+1)}, file=fvtk1)
        for k in range(nz+1):
            for j in range(np+1):
                for i in range(nr+1):
                    if(j==0):
                        print('%(0)e %(1)d %(2)e'%{'0':Rc_p[i],'1':0,'2':Z_p[k]}, file=fvtk1)
                    elif(j==1):
                        print('%(0)e %(1)e %(2)e'%{'0':Rc_p[i],'1':1e-6,'2':Z_p[k]}, file=fvtk1)
        ...
        print('DIMENSIONS %(0)d %(1)d %(2)d'%{'0':nr,'1':nz,'2':np}, file=fvtk2)
        print('POINTS %(0)d float'%{'0':nr*nz*np}, file=fvtk2)
        for j in range(nz):
            for i in range(nr):
                print('%(0)e %(1)d %(2)e'%{'0':0.5*(Rc_p[i]+Rc_p[i+1]),'1':0,'2':Z_p[j]}, file=fvtk2)
