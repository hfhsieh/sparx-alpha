...
def main(pfile,direc,xaxis,yaxis,zaxis,cen_xx,cen_yy,cen_zz,density,Vxx,Vyy,Vzz,n1,n2,level,position,pf2,npart,cl):
    ...
    if (writegrid):
        ...
        text = ["{:12.6e}".format(xaxis[i]) for i in ragen(n1[0]+1)]
        text = " ".join(text)
        print(text, file=f)
        print('        </DataArray>', file=f)
        print('        <DataArray type="Float32" Name="Y_COORDINATES" NumberOfComponents="1">', file=f)
        text = ["{:12.6e}".format(yaxis[j]) for j in ragen(n1[1]+1)]
        text = " ".join(text)
        print(text, file=f)
        print('        </DataArray>', file=f)
        print('        <DataArray type="Float32" Name="Z_COORDINATES" NumberOfComponents="1">', file=f)
        text = ["{:12.6e}".format(zaxis[k]) for k in ragen(n1[2]+1)]
        text = " ".join(text)
        print(text, file=f)
        print('        </DataArray>', file=f)
        print('      </Coordinates>', file=f)
        print('      <CellData>', file=f)
        print('        <DataArray type="Float32" Name="density" NumberOfComponents="1" format="ascii">', file=f)
        ...
        f.close()
        print('    <DataSet group="%(0)d" dataset="0" file="%(1)s"/>'%{'0':npart,'1':fname}, file=pf2)
    return (npart+1,cl)
