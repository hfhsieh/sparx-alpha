...
class vtk_output:
    ...
    def _vtk_sph1d(self,mesh,phys):
        ...
        print('DIMENSIONS %(0)d %(1)d %(2)d'%{'0':np+1,'1':nt+1,'2':nr+1}, file=fvtk1)
        ...
        for i in range(nr+1):
          for j in range(nt+1):
            for k in range(np+1):
                ...
                print('%(0)e %(1)e %(2)e'%{'0':x,'1':y,'2':z}, file=fvtk1)
        print('CELL_DATA %(0)d'%{'0':nr * nt * np}, file=fvtk1)
        print('SCALARS density float 1', file=fvtk1)
        print('LOOKUP_TABLE default', file=fvtk1)
        for i in range(nr):
          for j in range(nt):
            text = ["{:8.2e}".format(phys.n_H2[i])  for k in range(np)]
            text = " ".join(text)
        ...
        for i in range(nr):
          for j in range(nt):
            text = ["{:7.1f}".format(phys.T_k[i])  for k in range(np)]
            text = " ".join(text)
            print(text, file=fvtk1)
        print('VECTORS velocity float', file=fvtk1)
        for i in range(nr):
          for j in range(nt):
            for k in range(np):
                Vr = phys.V_gas[i][0]
                ...
                Vz = Vr * cos(theta_p[j])
                print('%(0)7.2e %(1)7.2e %(2)7.2e'%{'0':Vx,'1':Vy,'2':Vz}, file=fvtk1)
        fvtk1.close()
    ...
    def _vtk_sph2d(self,mesh,phys):
        ...
        for i in range(nr):
          for j in range(nt):
            text = ["{:8.2e}".format(phys.n_H2[i, j])  for k in range(np)]
            text = " ".join(text)
            print(text, file=fvtk1)
        print('SCALARS temperature float 1', file=fvtk1)
        print('LOOKUP_TABLE default', file=fvtk1)
        for i in range(nr):
          for j in range(nt):
            text = ["{:7.1f}".format(phys.T_k[i, j])  for k in range(np)]
            text = " ".join(text)
            print(text, file=fvtk1)
        print('VECTORS velocity float', file=fvtk1)
        for i in range(nr):
    ...
    def _vtk_sph3d(self,mesh,phys):
        ...
        for i in range(nr):
          for j in range(nt):
            text = ["{:8.2e}".format(phys.n_H2[i, j, k])  for k in range(np)]
            text = " ".join(text)
            print(text, file=fvtk1)
        print('SCALARS temperature float 1', file=fvtk1)
        print('LOOKUP_TABLE default', file=fvtk1)
        for i in range(nr):
          for j in range(nt):
            text = ["{:7.1f}".format(phys.T_k[i, j, k])  for k in range(np)]
            text = " ".join(text)
            print(text, file=fvtk1)
        print('VECTORS velocity float', file=fvtk1)
        for i in range(nr):
    ...
    def _vtk_cyl2d(self,mesh,phys):
        ...
        print('ASCII', file=fvtk1)
        print('DATASET STRUCTURED_GRID', file=fvtk1)
        print('DIMENSIONS %(0)d %(1)d %(2)d'%{'0':nz+1,'1':np+1,'2':nrc+1}, file=fvtk1)
        print('POINTS %(0)d float'%{'0':(nrc+1)*(np+1)*(nz+1)}, file=fvtk1)
        for i in range(nrc+1):
          for j in range(np+1):
            for k in range(nz+1):
                ...
                y = rc * sin(phi)
                print('%(0)e %(1)e %(2)e'%{'0':x,'1':y,'2':z}, file=fvtk1)
        print('CELL_DATA %(0)d'%{'0':nrc * np * nz}, file=fvtk1)
        print('SCALARS density float 1', file=fvtk1)
        print('LOOKUP_TABLE default', file=fvtk1)
        for i in range(nrc):
          for j in range(np):
            text = ["{:8.2e}".format(phys.n_H2[i, k])  for k in range(nz)]
            text = " ".join(text)
            print(text, file=fvtk1)
        print('SCALARS temperature float 1', file=fvtk1)
        print('LOOKUP_TABLE default', file=fvtk1)
        for i in range(nrc):
          for j in range(np):
            text = ["{:7.1f}".format(phys.T_k[i, k])  for k in range(nz)]
            text = " ".join(text)
            print(text, file=fvtk1)
        print('VECTORS velocity float', file=fvtk1)
        for i in range(nrc):
