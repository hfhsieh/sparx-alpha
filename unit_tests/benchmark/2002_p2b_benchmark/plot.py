...
### Model
data = np.genfromtxt("model_2.d", usecols = [0, 1, 3, 4, 5])
...
fig.tight_layout()
plt.savefig("P2B_model.png", dpi = 120, bbox_inches = "tight")
plt.close(fig)
...
for idx, ax in enumerate(axes_ravel[:-2]):
    ...
    if idx < 3:
        ax.text(0.1, 0.82, "HCO$^{+}$" + " J={}".format(idx),
                transform = ax.transAxes)
    else:
        ax.text(0.94, 0.88, "HCO$^{+}$" + " J={}".format(idx),
...
plt.savefig("P2B_pops.png", dpi = 120, bbox_inches = "tight")
plt.close(fig)
